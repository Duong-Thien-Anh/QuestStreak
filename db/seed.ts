...
import { ensureDefaultAchievements, ensureMemberProgress } from "../api/lib/gamification";
...
async function seed() {
  ...
  await ensureMemberProgress(db, adminMember.id);
  await ensureMemberProgress(db, subMember.id);
